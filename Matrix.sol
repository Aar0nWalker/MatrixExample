...
contract CryptoSpace{
    ...
    function registrationInt(address userAddr, address refAddr) private{
        ...
        emit Registration(userAddr, refAddr, lastUserId, users[refAddr].id);
    }
    ...
    function updateStructure(address userAddr, uint StrId, uint lvl) private{
        ...
        users[userAddr].AbsPlace[lvl]=lvlToStr[lvl].freeAbsPlace;
        users[userAddr].usersBP[lvl]=usersBeforeFirstPandingCounter(userAddr, lvl);
        ...
        emit NewUserPlace(userAddr, StrId, lvl, users[userAddr].place[lvl], users[userAddr].Insurance[lvl]);
    ...
    function sendRewards(address userAddr, uint256 StrId, uint lvl) private{
        ...
        if (users[userAddr].place[lvl]==11){
              ...
              users[receiver].usersBP[lvl]=0;
        }else{
              if (users[userAddr].place[lvl]==8){
                  receiver=payable(lvlToStr[lvl].idToStr[StrId].mainTableLevel);
                  users[receiver].usersBP[lvl]=3;
              }else{
                    if (users[userAddr].place[lvl]<8){
                    ...
                    users[receiver].usersBP[lvl]=usersBeforeSecondPandingCounter(receiver, lvl);
              }
        }
    ...
    uint extraBR, uint lostMoney, bool plevelWasActivate, bool levelIsActive, uint laceInStructure, uint absolutePlaceInLevel, uint usersBP, bool insurance) {
        address ad=userAddr;
        uint Lev=lvl;
        ...
        users[ad].AbsPlace[Lev], users[ad].usersBP[Lev], users[ad].Insurance[Lev]);
    }
